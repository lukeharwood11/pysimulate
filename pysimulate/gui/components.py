...
class ArrowKeysDisplay:
    ...
    def __init__(
        self, unselected_color, selected_color, border=None, scale=1, position=None
    ):
    ...
    def render(self, window, position, current_actions: list[int]):
        ...
        pygame.draw.rect(
            ...
            (
                ...
                if current_actions.count(0) == 0
                else self.selected_color
            ),
            self._left_key_rect.move(*position),
            border_radius=10,
        )
        pygame.draw.rect(
            window,
            (
                self.unselected_color
                if current_actions.count(1) == 0
                else self.selected_color
            ),
            self._up_key_rect.move(*position),
            border_radius=10,
        )
        pygame.draw.rect(
            window,
            (
                self.unselected_color
                if current_actions.count(2) == 0
                else self.selected_color
            ),
            self._right_key_rect.move(*position),
            border_radius=10,
        )
        pygame.draw.rect(
            window,
            (
                self.unselected_color
                if current_actions.count(3) == 0
                else self.selected_color
            ),
            self._down_key_rect.move(*position),
            border_radius=10,
        )
    ...
    def build_model(self) -> np.array:
        ...
        self._down_key_rect = pygame.rect.Rect(
            self.width, self.width, self.width, self.width
        )
        self._right_key_rect = pygame.rect.Rect(
            self.width * 2, self.width, self.width, self.width
        )
